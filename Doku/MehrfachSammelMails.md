...
Diese Dokumentation beschreibt die erweiterte Sammelfunktion für mehrere Aufträge. Die normale Sammel-E-Mail fasst bereits die Dateien eines einzelnen Auftrags zusammen, zum Beispiel Rechnung, Nachweis bzw. Prothetikpass und KZBV-XML. Die Mehrfach-Sammel-E-Mail erlaubt zusätzlich, mehrere vollständige Aufträge nacheinander zu sammeln und erst am Ende eine gemeinsame E-Mail mit allen Anhängen zu erzeugen.
...
Beim Druck eines Auftrags wird die E-Mail nicht sofort erzeugt. Delapro erzeugt zuerst die benötigten Dateien und legt sie in einen Sammeljob. Nach jedem Auftrag fragt Delapro, ob weitere Aufträge hinzugefügt werden sollen oder ob die E-Mail jetzt erzeugt werden soll.

Je nach Auftrag und Einstellung können gesammelt werden:
...
Die Auswahl bei `Sammeln` hat folgende Bedeutung:

| Auswahl | Bedeutung |
|---|---|
| `Auftrag` | Die Dateien des aktuellen Auftrags werden gesammelt und danach direkt als E-Mail ausgegeben. Das ist der bisherige Sammelmodus. |
| `Mehrfach` | Die Dateien mehrerer Aufträge desselben Kunden werden gesammelt, bis der Benutzer die E-Mail erzeugt oder den Sammeljob verwirft. |
| `Nein` | Sammeln wird für diesen Druckvorgang ausgeschaltet. |

### Anzeige eines aktiven Sammelvorgangs

Solange ein Mehrfach-Sammeljob aktiv ist, zeigt die Auftragsverwaltung im Fenstertitel rechts eine kurze Statusanzeige an, zum Beispiel:

```text
Sammlung 3 Dat. Kd. 123
```

Die Zahl zeigt die bisher gesammelten Dateien, nicht zwingend die Anzahl der Aufträge. Ein Auftrag kann mehrere Dateien erzeugen, zum Beispiel Rechnung, Nachweis und XML-Datei.

Die Kundennummer wird angezeigt, sobald sie für den Sammeljob feststeht. Dadurch ist erkennbar, für welchen Kunden gerade gesammelt wird.

### Ablauf beim Sammeln mehrerer Aufträge

Nach dem Druck eines Auftrags erscheint eine Abfrage sinngemäß:
...
Nach erfolgreichem Abschluss wird die Mehrfach-Vorgabe am verwendeten Druckertreiber wieder zurückgesetzt. Beim nächsten Aufruf steht der Sammelmodus dadurch wieder auf dem normalen Auftragssammeln und nicht mehr auf `Mehrfach`.
...
Auch beim Verwerfen wird die Mehrfach-Vorgabe am verwendeten Druckertreiber wieder zurückgesetzt.
...
### Abbruch und Verlassen der Auftragsverwaltung

Wird die Auftragsverwaltung verlassen, obwohl noch ein Mehrfach-Sammeljob aktiv ist, fragt Delapro den Sammeljob nochmals ab. Der Benutzer kann dann die E-Mail erzeugen, weiter sammeln oder verwerfen.

Bleibt nach dem Verlassen immer noch ein aktiver Sammeljob übrig, wird nochmals das Verwerfen angeboten. Dadurch soll vermieden werden, dass ein offener Sammeljob unbemerkt stehen bleibt.

Ein begonnener Sammeljob sollte immer bewusst abgeschlossen oder verworfen werden.
...
### F7 Optionen während eines aktiven Mehrfach-Sammeljobs

Wenn ein Mehrfach-Sammeljob bereits Druckdaten enthält, können die F7-Sammeloptionen nicht einfach umgestellt werden. Delapro weist dann darauf hin, dass der Sammeljob zuerst ausgegeben oder weitergeführt werden muss.

Damit soll verhindert werden, dass ein laufender Sammeljob durch eine geänderte Druckertreibervorgabe in einen unklaren Zustand gerät.
...
| `Mehrfach` | neuer Modus: Dateien mehrerer Aufträge desselben Kunden werden gesammelt, bis der Benutzer die E-Mail erzeugt oder den Sammeljob verwirft |
| `Nein` | Sammeln für diesen Druckvorgang deaktivieren |

Die Auswahl `Mehrfach` ist als temporäre Bedienoption gedacht. Nach erfolgreicher Ausgabe oder nach dem Verwerfen eines Mehrfach-Sammeljobs setzt Delapro die Vorgabe für den verwendeten Druckertreiber wieder auf das normale Auftragssammeln zurück.

### Kundenparameter und Sammeloption Labor / Ein / Aus

Für den gemeinsamen Versand sind weiterhin die PDF-/E-Mail-Einstellungen des Kunden maßgeblich:
...
Die Dateien werden mit einem passenden Präfix erzeugt, zum Beispiel aus `DLPAuftrag` oder `DLPNachweis`. Die sichtbaren Anhangsnamen in der E-Mail werden weiterhin aus der Betreff-/PDF-Logik gebildet.

### Technischer Ablauf in Kurzform

Der Mehrfach-Sammeljob wird in der Auftragsverwaltung gehalten. Solange der Job aktiv ist, wird er bei weiteren `F6`-Druckvorgängen wiederverwendet. Die eigentliche Drucklogik erzeugt je Auftrag die benötigten Dateien und trägt sie in den Job ein.

Im Mehrfachmodus schließt `AVD_Drucken()` den Job nicht selbst ab. Erst die Benutzerentscheidung `E-Mail erzeugen` ruft den finalen Abschluss auf. Danach werden die gesammelten Dateien in die E-Mail-Metadatei aufgenommen und die externe Ausgabe gestartet.

Beim Abschluss oder Verwerfen wird der Job geleert und die temporäre Mehrfach-Vorgabe am Druckertreiber wieder auf Auftragssammeln gesetzt.
...
5. Verlassen der Auftragsverwaltung mit aktivem Sammeljob.
6. F7-Optionen während eines bereits aktiven Mehrfach-Sammeljobs.
7. Auftrag mit Rechnung, Nachweis und KZBV-XML.
8. Arbeitsplatz im Netzwerk, auf dem die PDF-/Ghostscript-Ausgabe tatsächlich produktiv verwendet wird.
